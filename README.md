...
- **文档解析**（@firecrawl/anydoc）：支持 `.doc/.docx/.docm .ppt/pptx/.pps/.pot/.pptm/.ppsx/.ppsm .xls/.xlsx/.xlsm/.xlsb .odt/.ods/.odp .rtf .epub .csv .pdf`；纯文本（`.json/.txt/.md/.html/.shtml`）直读（UTF-8 含 BOM 优先、GB18030/GBK 兜底，JSON 美化、HTML 剥标签）；PDF 含图片时不中断，给出可读降级说明。
...
多模态模型**只从「设置-模型」中选择**（唯一配置来源）：设置页「DshOmniFile」下拉里选择支持 image 的提供商/模型，保存一条 `providerRef` 引用，不在此保存多份模型配置。

## 注意事项

- **多模态模型需先在「设置-模型」配置**并支持 image 输入，否则图片/文档内嵌图片识别会报错。
