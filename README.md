...
5. **聊天内文件卡片**
   - 发送后聊天里在**用户消息上方**显示文件卡片（图标 + 文件名），点击通过 DSH 的 host.openPath
   ...
   - 文档/文本（含未枚举但可按文本读取的扩展名，如 .js/.ts/.py...）显示「已读取/已解析」**静态卡片**，
     与用户消息一样**右对齐**，双行均为原先「展开/收缩」行的样式（不再支持展开/收缩）：
     - 行1（图标 + 文件名 + 已解析）：右侧 **📂 按钮 = 打开「源文件」**（转换前的原始文件，
       本地默认程序打开；标记尾部附带的 `（源文件：<源路径>）` 用于精确回指原文件）；
     - 行2（「解析后保存路径：<md文件名>」）：同样 row 样式，点击用本地默认程序打开**转换出的 md**
       预览，完整绝对路径在悬停 title 中（GUI 消息正文为纯文本渲染，无法内嵌可点击 HTML 链接，
       故该路径放在卡片上）；
     - **用户消息气泡内不再显示任何解析信息**：marker 仍保留在消息 content 中（供大模型
       read/read_image 等工具按该路径直接读取），气泡里由客户端按 DOM 把 marker 段视觉隐藏，
       只显示用户的原始输入。
...
- **聊天卡片标记（无需 token）**：发送文档/文本后，消息正文用**可读文本**标记
  ...
  “已读取/已解析”静态卡片显示在该条用户消息的**上方**（用户消息的前一个位置）。
  卡片从正文提取文件名/类别/保存路径（“保存路径”即 `{源文件名}.md` 的绝对路径；
  尾部 `（源文件：…）` 供 📂 按钮精确打开**源文件**；行2可点击的 md 路径链接用于打开解析结果；
  不向消息里塞任何 `[[omnifile:...]]` 之类的无意义 token），
  **消息气泡内不再渲染这段 marker**（客户端按 DOM 视觉隐藏），模型仍能从 content 里读到该路径。
...
- **消息只放引用、内容在卡片**：为保持对话栏整洁，发送时仅写入上述**一行可读引用**到消息 content
  ...
  `{源文件名}.md`，需要精确原文时点开卡片行2/📂 本地打开，模型则按保存路径 read 获取。
