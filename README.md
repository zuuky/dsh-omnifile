...
4. **发送到主模型**
   ...
   - **发送时等待解析完成**：点击发送时若还有文件未解析完，会等所有文件解析完成后再发出消息——
     因为解析早已在选中后开始，绝大多数情况下发送是秒开的，不会阻塞。
   - 非图片文件发送时，把 Markdown / 图片描述 / 保存路径组合为文本发给主模型
     （文本-only / 多模态主模型都能识别）。
...
5. **聊天内文件卡片**
   ...
   - 解析结果统一落盘为 **`{源文件名}.md`**（保持原名再追加 `.md`，如 `报告.docx` → `报告.docx.md`、
     `utils.js` → `utils.js.md`），保存到会话工作区 `uploads/`；消息标记里的「保存路径」就是该 md 的
     **绝对路径**，方便大模型直接对该路径触发内置 `read` 工具获取解析内容。
   - 文档/文本（含未枚举但可按文本读取的扩展名，如 .js/.ts/.py...）显示「已读取/已解析」卡片，
     与用户消息一样**右对齐**：
     - 标签右侧 **📂 按钮 = 打开「源文件」**（转换前的原始文件，本地默认程序打开；标记尾部附带的
       `（源文件：<源路径>）` 用于精确回指原文件）；
     - 卡片下方「解析后保存路径」以**可点击链接**展示（真实 `<a>` 锚点），点击用本地默认程序打开
       **转换出的 md** 预览（GUI 消息正文为纯文本渲染，无法内嵌可点击 HTML 链接，故链接放在卡片上）；
     - 点击卡片其余区域可展开/折叠完整解析内容。
   - 无法按文本读取的格式（二进制/APK/ZIP 等）显示为可点击的文件卡片，点击即本地打开源文件。
...
| GET  /api/omnifile/parsed | 按保存路径返回解析结果全文（`<uploads>/<源文件名>.md`，折叠卡片懒加载用；路径本身带 `.md` 时直接读，旧标记则按规则推导） |
...
- **host 端改动需重启生效**：client 端（lib/client.js）改动由 DSH 客户端 HMR 即时热替换、无需重启；
  host 端（lib/index.js）新增/修改的路由与逻辑需重启 DSH（或 dsh web）后注册。本轮涉及 host 端
  改动（解析结果改落盘为 `{源文件名}.md`、`/parsed` 支持直接读 md、`/process` 接收 `name` 定名）——
  **升级后请重启一次再使用**，避免新客户端标记与旧宿主解析命名不一致。
...
- **聊天卡片标记（无需 token）**：发送文档/文本后，消息正文用**可读文本**标记
  ...
  卡片从正文提取文件名/类别/保存路径（“保存路径”即 `{源文件名}.md` 的绝对路径；
  尾部 `（源文件：…）` 供 📂 按钮精确打开**源文件**；成功时卡片下方展现可点击的 md 路径链接；
  不向消息里塞任何 `[[omnifile:...]]` 之类的无意义 token），
  点击展开时由宿主 `/api/omnifile/parsed` 按保存路径懒加载完整解析内容。
- **标记格式为宿主导（index.js）与客户端（client.js）双份常量**：改动状态词/保存路径前缀/源文件标签时
  需同步两处（各自集中在文件顶部 MARKER_* 常量区），避免改不全导致卡片漏抽。
- **消息只放引用、内容在卡片**：为保持对话栏整洁，发送时仅写入上述**一行可读引用**，不把解析内容全文
  重复填充进对话栏；完整解析内容放在聊天内折叠卡片里（点击展开，由 `/api/omnifile/parsed` 懒加载），
  需要精确原文时点开卡片即可，也可用 📂 按钮本地打开源文件。
