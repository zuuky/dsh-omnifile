...
5. **聊天内文件卡片**
   ...
   - 文档/文本（含未枚举但可按文本读取的扩展名，如 .js/.ts/.py...）显示「已读取/已解析」卡片，
     与用户消息一样**右对齐**，标签右侧带 **📂 打开**按钮（本地默认程序打开**源文件**原样，
     标记尾部附带的 `（源文件：<源路径>）` 用于精确回指原文件），
...
- **聊天卡片标记（无需 token）**：发送文档/文本后，消息正文用**可读文本**标记
  （`【文档「报告.docx」·保存路径：<uploads>/报告.docx.md】已解析（源文件：<源绝对路径>） · N 字符（完整内容见上方文件卡片，可点击展开）`），
  “已读取/已解析”卡片显示在该条用户消息的**上方**（用户消息的前一个位置）。
  卡片从正文提取文件名/类别/保存路径（“保存路径”即 `{源文件名}.md` 的绝对路径；
  尾部 `（源文件：…）` 是可选原文件回指，供 📂 按钮精确打开源文件；
