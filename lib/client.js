window.__ModuleLoader__.load({
    ...
    factory: (require) => {
        ...
        const CSS = [
            ...
            /* 修复：输入框内文件 chip 现在以可见 label（文件名）呈现；
             * 不再隐藏空 label，避免 textarea 中 U+FFFC 原本体裸露成"隐形占位"。 */
            ...
            /* 修复1：保存按钮使用 DSH 官方主按钮配色对（background=button-primary-fill，
            ...
            /* 修复2：聊天内「已解析文件内容」折叠卡片（类 Think 折叠） */
        ...
        const TOKEN_RE = /\[\[omnifile:([A-Za-z0-9_-]+)\]\]/g;

        function manifestToken(record) {
            return '[[omnifile:' + b64urlEncode(JSON.stringify({
                name: record.name,
                path: record.path,
                kind: record.kind,
                size: record.size,
                mime: record.mime,
                ...(record.parsed !== undefined && record.parsed !== null && record.parsed !== '' ? {parsed: record.parsed} : {}),
            })) + ']]';
        }
        ...
        function hasOmnifileTokens(content) {
            TOKEN_RE.lastIndex = 0;
            return TOKEN_RE.test(textOf(content));
        }
        ...
        class OmnifileController {
            ...
            async addNonImage(sessionId, input, file) {
                ...
                /* 修复：占位符改为可见文件 chip（label=文件名）。
                 * 之前 label 置空 → backdrop 隐藏 chip → textarea 里的 U+FFFC 原本体裸露，
                 * 表现为"看不见的占位"：光标无法从最前面输入、按删除会静默删除占位并连带取消文件。
                 * 现在 chip 可见，用户能看到附件位置，删除 chip 为显式的主动操作。 */
            ...
            async addFiles(sessionId, files) {
                ...
                /* 修复3：所有非图片文件（含未知格式）都走上传+解析；未知格式由 host 按文本读取，
                 * 读不了会提示用户，不再直接拒绝。 */
            ...
            async serialize(ref, signal) {
                ...
                try {
                    const response = await fetch('/api/omnifile/process', {
                        method: 'POST',
                        headers: {'content-type': 'application/json'},
                        body: JSON.stringify({sessionId: record.sessionId, path: record.path, kind: record.kind}),
                    ...
                    record.status = 'done';
                    if (typeof json.parsedPath === 'string' && json.parsedPath !== '') record.parsed = json.parsedPath;
                    this.changed();
                    // const token = manifestToken(record);
                    ...
                    if (json.kind === 'doc' || json.kind === 'text') {
                        ...
                        return '【文件「' + record.name + '」已解析 · ' + chars + ' 字符（内容已折叠，见下方卡片）】\n\n'
                            + (summary !== '' ? '摘要：' + summary + '\n\n' : '')
                    }
                    ...
                    this.changed();
                    return '【文件「' + record.name + '」】绝对路径：' + record.path + '（自动解析失败：' + messageOf(error) + '）' + '\n' + manifestToken(record);
                }
            }
            ...
            async loadParsed(sessionId, file) {
                ...
                if (typeof parsed !== 'string' || parsed === '') throw new Error('没有可加载的解析内容');
                const response = await fetch('/api/omnifile/file?sessionId=' + encodeURIComponent(String(sessionId || ''))
                    + '&path=' + encodeURIComponent(parsed));
        ...
        function OmnifileDock(props) {
            ...
            return react.createElement('div', {className: 'omnifile-dock', role: 'status', 'aria-label': '已附加文件'},
                occurrences.map(function (occurrence) {
                    ...
                    const detail = record.status === 'processing' ? '解析中…'
        ...
        function OmnifileFilesCard(props) {
            ...
            return react.createElement('div', {className: 'omnifile-parse-block'},
                files.map(function (file) {
                    const key = file.path + '|' + (file.parsed || '');
                    if (typeof file.parsed === 'string' && file.parsed !== '') {
        ...
        function ParseBlock(props) {
            ...
            const summary = file.kind === 'text' ? '已读取'
                : file.kind === 'doc' ? (file.size !== undefined ? humanBytes(file.size) + ' · 已解析' : '已解析')
        ...
        function omnifileChatDefinition() {
            return {
                ...
                match: function (event) {
                    ...
                    if (!hasOmnifileTokens(event.data.content)) return null;
                ...
                buildViewNode: function (context) {
                    ...
                    return chatNode(context, 'omnifile-files', context.state.seq, context.state);
                },
            };
        }
                ...
                return ctx.slots.register({
                    ...
                    inject: function (sessionId) {
                        return {
                            ...
                            /* 任务6：点击 dock 缩略图/文件卡片 → 用系统默认程序预览 */
