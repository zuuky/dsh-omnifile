window.__ModuleLoader__.load({
    ...
    factory: (require) => {
        ...
        const SOURCE = '文件';
        /* ════════════════════════════════════════════════════════════════════
         * 统一常量（Req4：公共字符串/中文文案/组装集中收拢，避免改不全）。
         * 注意：宿主 lib/index.js 也有「消息标记」常量（Unable to import 宿主模块），
         * 改动下方的 MARKER_* 字符串时必须与宿主 index.js 同步，否则卡片解析会漏抽。
         * ════════════════════════════════════════════════════════════════════ */
        ...
        /* 组装「解析后保存路径」标记（与宿主 markerText 同形；source 覆盖时自动追加源文件）。 */
        function markerText(path, source, status, note) {
            if (status === MARKER_STATUS_OK) {
            ...
            return MARKER_PREFIX + path + '（' + status + (note ? '：' + note : '') + '）';
        }
        /* UI 文案（集中收拢，避免改不全）。 */
        const LBL_OPEN_SOURCE = '用本地默认程序打开源文件';
        const LBL_OPEN_PARSED = '用本地默认程序打开解析结果';
        const LBL_CHIP_OPEN = '（点击预览）';
        const LBL_ADD_FILES = '添加本地文件（可多选，支持拖拽/粘贴）';
        const LBL_LOADING = '加载中...';
        const LBL_LOAD_FAILED = '加载失败：';
        const LBL_PARSED = '已解析';
        /* 客户端限额缺省值（Req3：实际值启动时由 /api/omnifile/config 覆盖）。 */
        ...
        const CSS = [
            ...
            /* 卡片内「解析后保存路径」可点击链接（Req2） */
            '.omnifile-parse-mdrow{display:flex;align-items:center;gap:4px;min-width:0;padding:0 12px 6px;font-size:11px;line-height:1.5;color:var(--dsw-alias-label-tertiary,#888);}',
            '.omnifile-parse-mdlabel{flex:none;}',
            '.omnifile-parse-md{display:inline-flex;align-items:center;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--dsw-alias-brand-primary,#4b6bfb);text-decoration:none;cursor:pointer;font-family:ui-monospace,SFMono-Regular,Consolas,monospace;}',
            '.omnifile-parse-md:hover{text-decoration:underline;}',
            '.omnifile-parse-md:focus-visible{outline:2px solid var(--dsw-alias-brand-primary,#4b6bfb);outline-offset:2px;border-radius:4px;}',
         ...
         * 正则的状态分支用 MARKER_* 常量动态生成，新增状态只需改常量（Req4）。
         * 卡片据此提取保存路径、源文件与类别；正则要求 （ 后紧跟本插件的状态词，避免把内容里形似的字符串误抽。 */
        const PARSE_STATUS_RE = '(?:' + MARKER_STATUS_OK + '|' + MARKER_STATUS_UNREADABLE + '|' + MARKER_STATUS_FAILED + '|' + MARKER_UNKNOWN + ')';
        /* 组1=保存路径，组2=状态词+尾巴（含「；源文件：…」），供 sourcePathOf 提取源文件（Req1）。 */
        const PARSE_RE = new RegExp(MARKER_PREFIX + '(.+?)（(' + PARSE_STATUS_RE + '[^）]*)）', 'g');
        const PARSE_MARKER_RE = new RegExp(MARKER_PREFIX + '.+?（(' + PARSE_STATUS_RE + '[^）]*)）');
        /** 从状态尾巴（如「完整内容见…；源文件：D:\x」）提取源文件绝对路径；没有则返回未定义。 */
        ...
        function extractFiles(content) {
            ...
            while ((m = PARSE_RE.exec(text)) !== null) {
                ...
                seenPaths[path] = true;
                files.push({name: name || '文件', kind: parsed ? 'doc' : 'other', path, sourcePath});
            }
            return files;
        }
        ...
        class OmnifileController {
            constructor(ctx) {
                ...
                /* Req3：客户端限额从宿主 /api/omnifile/config 读取，避免与 settings 不同步。 */
             ...
             * 成功时保存路径即 {源文件名}.md 绝对路径，模型可用内置 read 直接读取；源文件为原始文件路径（Req1）。 */
            async serialize(ref, signal) {
                ...
                if (json !== null && json.ok === true) {
                    ...
                    if (json.kind === 'other') {
                        ...
                        return markerText(p, '', MARKER_STATUS_UNREADABLE, json.note || '' ) + '\n';
                    }
                    /* image / doc / text 命中 → 有 md（{源文件名}.md），只放一行引用，完整内容在卡片里；
                     * 附「源文件」回指，客户端 📂 与卡片据此打开原始文件（Req1）。 */
                    return markerText(p, record.path, MARKER_STATUS_OK) + '\n';
                }
                return markerText(record.path, '', MARKER_STATUS_FAILED, record.error || '') + '\n';
            }
                ...
                files.map(function (file) {
                    ...
                    return react.createElement('div', {key: key, className: 'omnifile-chat-files'},
                        react.createElement('button', {
                                ...
                                title: (file.sourcePath || file.path) + '（点击用本地默认程序打开原始文件）',
                                onClick: function () {
         ...
         * Req1：📂 打开「源文件」（原始文件，而非解析出的 md）；Req2：解析后保存路径以
         * 可点击的 <a>（真实锚点）展示，点击用本地默认程序打开 md 预览。 */
        function ParseBlock(props) {
            ...
            const summary = LBL_PARSED;
            const title = file.name;
            const mdName = basenameOf(file.path);
            /* 仅当 md 存在（解析成功）时渲染可点击 md 链接；sourcePath !== file.path 即成功场景。 */
            const mdLink = file.path && file.sourcePath !== undefined && file.sourcePath !== file.path
                ? react.createElement('span', {className: 'omnifile-parse-mdrow'},
                    react.createElement('span', {className: 'omnifile-parse-mdlabel'}, MARKER_PREFIX),
                    react.createElement('a', {
                        className: 'omnifile-parse-md',
                        href: '#',
                        title: file.path + '（' + LBL_OPEN_PARSED + '）',
                        'aria-label': LBL_OPEN_PARSED + '：' + mdName,
                        onClick: function (ev) {
                            ev.preventDefault();
                            ev.stopPropagation();
            ...
            return react.createElement('div', {className: 'omnifile-parse-block'},
                ...
                open
                    ? react.createElement('div', {className: 'omnifile-parse-body'},
                        error !== null
                            ? react.createElement('div', {className: 'omnifile-parse-loading'}, LBL_LOAD_FAILED + error)
                            : loading
                                ? react.createElement('div', {className: 'omnifile-parse-loading'}, LBL_LOADING)
        ...
        function OmnifileSettings(props) {
            ...
            /* 数字上限字段（Req4：设置面板内的通用数值输入）。mb=true 时以 MB 展示/落盘。 */
            const numField = function (label, key, fallback, min, step, hint, mb) {
                return field(label, react.createElement('input', {
                    className: 'omnifile-cfg-input',
                    type: 'number',
                    min: min,
                    step: step,
                    value: value[key] === undefined || value[key] === null ? fallback : (mb ? Number(value[key]) / (1024 * 1024) : Number(value[key])),
                    onChange: function (e) {
                        const n = parseFloat(e.target.value);
                        const raw = Number.isFinite(n) ? (mb ? Math.round(n * 1024 * 1024) : n) : fallback;
                        update([key], raw);
                    },
            ...
            return react.createElement('div', {className: 'omnifile-cfg'},
                ...
                react.createElement('hr', {className: 'omnifile-cfg-divider'}),
                /* 限制参数（Req3：原为代码内硬编码常量，现可在设置界面配置） */
                react.createElement('div', {className: 'omnifile-cfg-group'}, [
