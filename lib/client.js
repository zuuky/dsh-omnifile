window.__ModuleLoader__.load({
    ...
    factory: (require) => {
        ...
        const CSS = [
            ...
            /* 聊天内「已解析」静态卡片（Req1：不再支持展开/收缩；
             * 行样式与原先的展开/收缩行 parse-row 完全一致）。 */
            '.omnifile-parse-block{display:flex;flex-direction:column;gap:4px;max-width:640px;width:fit-content;margin-left:auto;align-self:flex-end;}',
            ...
            '.omnifile-parse-row{box-sizing:border-box;display:flex;align-items:center;gap:8px;height:30px;max-width:100%;padding:0 10px;border:1px solid var(--dsw-alias-border-l1,rgba(0,0,0,.12));border-radius:10px;background:var(--dsw-specific-tip,rgba(128,128,128,.08));color:var(--dsw-alias-label-primary);font-size:13px;line-height:1;user-select:none;}',
            ...
            /* 「解析后保存路径」行：同样 parse-row 样式，标签次要色 + 路径链接主色可点击（Req1/Req2） */
            '.omnifile-parse-mdrow{background:var(--dsw-specific-tip,rgba(128,128,128,.08));}',
            '.omnifile-parse-mdlabel{flex:none;color:var(--dsw-alias-label-tertiary,#888);font-size:12px;white-space:nowrap;}',
            '.omnifile-parse-md{display:inline-flex;align-items:center;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--dsw-alias-brand-primary,#4b6bfb);font-family:ui-monospace,SFMono-Regular,Consolas,monospace;}',
            ...
            /* Req2：用户消息气泡里的 marker 段（保留在 content 供模型 read/read_image 用），
        ...
        function OmnifileFilesCard(props) {
            ...
            return react.createElement('div', {className: 'omnifile-chat-group'},
                files.map(function (file) {
                    const key = file.path;
                    /* 文档/文本一定有解析结果 → 静态「已解析」卡片；其余（未知格式）仅展示可点击的文件卡片。 */
        ...
        /** 单个文件的「已解析」静态卡片（Req1/Req2）：不再支持展开/收缩，也不在消息气泡里
         * 显示任何解析信息。与用户消息右对齐，行样式与原先的展开/收缩行（parse-row）一致：
         *  Row1：图标 + 文件名 + 已解析 + 📂（用本地默认程序打开源文件）——静态；
         *  Row2：「解析后保存路径：<md文件名>」——同样 parse-row 样式，点击用本地默认程序
         *        打开解析结果 md（完整路径在 title/aria-label，消息内容里的 marker 保留给模型
         *        read/read_image 用，气泡里已由 installMarkerHiding 视觉隐藏）。 */
        function ParseBlock(props) {
            const file = props.file;
            const sourcePath = file.sourcePath || file.path;
            const mdName = basenameOf(file.path);
            /* 仅当 md 存在（解析成功）时渲染「解析后保存路径」行；sourcePath !== file.path 即成功场景。 */
            const mdRow = (file.path && file.sourcePath !== undefined && file.sourcePath !== file.path)
                ? react.createElement('div', {
                    className: 'omnifile-parse-row omnifile-parse-mdrow',
                    role: 'button',
                    tabIndex: 0,
                    ...
                    react.createElement('span', {className: 'omnifile-parse-icon'}, '📄'),
                    react.createElement('span', {className: 'omnifile-parse-mdlabel'}, common.MARKER_PREFIX),
                    react.createElement('span', {className: 'omnifile-parse-md'}, mdName),
                )
                : null;
            return react.createElement('div', {className: 'omnifile-parse-block'},
                /* Row1：静态文件行（不再可点击展开） */
                react.createElement('div', {className: 'omnifile-parse-row'},
                    ...
                    react.createElement('span', {className: 'omnifile-parse-summary'}, LBL_PARSED),
                    react.createElement('button', {
                        type: 'button',
        ...
        /* Req2：用户消息气泡里不出现任何文件解析信息（marker 仍保留在 content 中，
        ...
        function apply(ctx) {
            ...
            /* Req2：隐藏用户消息气泡里的 marker（保留在 content 供模型工具识别）。 */
