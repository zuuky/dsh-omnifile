    ...
    factory: (require) => {
        ...
        function extractFiles(content) {
            ...
            while ((m = PARSE_RE.exec(text)) !== null) {
                ...
                seenPaths[path] = true;
                files.push({name: String(m[2] || ''), kind: kindByLabel[m[1]] || 'other', path});
            }
            return files;
        }
        ...
        class OmnifileController {
            ...
            async serialize(ref, signal) {
                ...
                try {
                    ...
                    this.changed();
                    if (json.kind === 'image') {
                        return '【图片「' + record.name + '」·保存路径：' + record.path + '】\n' + (json.text || '') + '\n';
                    }
                    if (json.kind === 'doc' || json.kind === 'text') {
                        const label = json.kind === 'doc' ? '文档' : '文本';
                        const chars = String(json.markdown || '').length;
                        /* 只放可读引用、不重复填充对话栏：完整内容在聊天卡片里（点击展开由 /parsed 懒加载），
                         * 卡片据此提取文件名/类别/保存路径。 */
                        return '【' + label + '「' + record.name + '」·保存路径：' + record.path + '】已解析 · ' + chars + ' 字符（完整内容见上方文件卡片，可点击展开）\n';
                    }
                    return '【文件「' + record.name + '」·保存路径：' + record.path + '】无法按文本读取' + (json.note ? '：' + json.note : '') + '\n';
                } catch (error) {
                    stopPoll();
                    record.status = 'error';
                    ...
                    record.progressDetail = '';
                    this.changed();
                    return '【文件「' + record.name + '」·保存路径：' + record.path + '】解析失败：' + (messageOf(error) || '') + '\n';
                } finally {
                    stopPoll();
                }
            }
