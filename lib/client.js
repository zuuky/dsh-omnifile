...
function markerText$1(path, options = {}) {
  const p = String(path || "");
...
function sourcePathOf$1(statusTail) {
...
function rebuildParsers() {
  PARSE_STATUS_RE = "(?:" + common.MARKER_STATUS_OK + "|" + common.MARKER_STATUS_UNREADABLE + "|" + common.MARKER_STATUS_FAILED + "|" + common.MARKER_UNKNOWN + ")";
...
function OmnifileSettings(props) {
  ...
  if (scope === void 0) return React__namespace.createElement("div", { className: "omnifile-hint" }, "设置服务不可用。可在 $DSH_HOME/settings.yaml 的 " + common.NAMESPACE + ": 小节配置。");
...
function omnifileChatDefinition() {
  return {
    ...
    start: function(context, match, reader) {
      ...
      if (messageId !== "" && startedCards.has(messageId)) return void 0;
...
function installMarkerHiding(ctx) {
  ...
  const scanNode = function(root) {
    ...
    if (root.textContent === void 0 || root.textContent.indexOf(common.MARKER_PREFIX) < 0) return;
  ...
  const hideMarkerInText = function(textNode) {
    ...
    const statusGroup = "(?:" + escapeRegExp(common.MARKER_STATUS_OK) + "|" + escapeRegExp(common.MARKER_STATUS_UNREADABLE) + "|" + escapeRegExp(common.MARKER_STATUS_FAILED) + "|" + escapeRegExp(common.MARKER_UNKNOWN) + ")";
    const markerRe = new RegExp("(\\r?\\n)?" + escapeRegExp(common.MARKER_PREFIX) + "(.+?)（(" + statusGroup + "[^）]*)）(\\r?\\n)?", "g");
    const ranges = [];
    let m;
