window.__ModuleLoader__.load({
    ...
    factory: (require) => {
         ...
         *   解析后保存路径：<绝对路径>（<状态说明>）
         * 状态说明三选一：完整内容见上方文件卡片，可点击展开（成功，路径为 {源文件名}.md）/
         *   无法按文本读取：… / 解析失败：…（失败，路径为源文件）。
         * 卡片据此提取保存路径与类别；正则要求 （ 后紧跟本插件的状态词，避免把内容里形似的字符串误抽。 */
        const PARSE_RE = /解析后保存路径：(.+?)（(完整内容见上方文件卡片，可点击展开|无法按文本读取|解析失败|未知原因)[^）]*）/g;
        const PARSE_MARKER_RE = /解析后保存路径：.+?（(完整内容见上方文件卡片，可点击展开|无法按文本读取|解析失败|未知原因)[^）]*）/;
        ...
        /** 从消息正文提取「已解析文件」清单（保存路径/显示名/类别），按保存路径去重。 */
        ...
        class OmnifileController {
             ...
             * 统一格式：解析后保存路径：<md 或源路径>（完整内容见上方文件卡片，可点击展开 | 无法按文本读取：… | 解析失败：…）
             * 成功时保存路径即 {源文件名}.md 绝对路径，模型可用内置 read 直接读取。 */
            async serialize(ref, signal) {
                ...
                if (json !== null && json.ok === true) {
                    ...
                    if (json.kind === 'other') {
                        return '解析后保存路径：' + p + '（无法按文本读取' + (json.note ? '：' + json.note : '') + '）\n';
                    }
                    /* image / doc / text 命中 → 有 md（{源文件名}.md），只放一行引用，完整内容在卡片里 */
                    return '解析后保存路径：' + p + '（完整内容见上方文件卡片，可点击展开）\n';
                }
                return '解析后保存路径：' + record.path + '（解析失败：' + (record.error || '') + '）\n';
            }
