        ...
        class OmnifileController {
            ...
            async serialize(ref, signal) {
                ...
                try {
                    ...
                    record.progressDetail = '';
                    this.changed();
                    if (json.kind === 'image') return '【' + record.name + '】' + '\n' + (json.text || '') + '\n\n';
                    if (json.kind === 'doc' || json.kind === 'text') {
                        ...
                        return '【' + record.name + ' · 已解析 · ' + chars + ' 字符（内容已折叠，点击上方文件卡片可展开）】\n'
                            + (summary !== '' ? '内容摘要：' + summary + '\n' : '') + '保存路径：' + record.path + '\n\n';
                    }
                    return '【' + record.name + '】 · 解析失败：' + (json.note || '') + '\n保存路径：' + record.path;
                } catch (error) {
                    stopPoll();
                    record.status = 'error';
                    ...
                    record.progressDetail = '';
                    this.changed();
                    return '【' + record.name + '】 · 解析失败：' + (messageOf(error) || '') + '\n保存路径：' + record.path;
                } finally {
                    stopPoll();
                }
            }
