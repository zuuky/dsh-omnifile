...
export function apply(ctx, config = {}) {
    ...
    if (tools !== undefined) {
        ctx.effect(() => tools.register(defineTool({
            name: 'dshomnifile',
            description: "分析本地文件并把解析结果落盘为 {源文件名}.md，返回一行可读路径引用（解析后保存路径：<md绝对路径>（完整内容见上方文件卡片，可点击展开））；如解析失败/无法读取则返回「解析后保存路径：<源路径>（解析失败/无法按文本读取：原因）」。拿到 md 路径后用内置 read 工具即可读取完整内容。支持的类型与行为：\n" +
             ...
             *   解析后保存路径：<md 或源路径>（完整内容见上方文件卡片，可点击展开 | 无法按文本读取：… | 解析失败：…）
             */
            async execute(args, exec) {
                ...
                const ok = async (markdown) => {
                    ...
                    return '解析后保存路径：' + (mdPath || path) + '（完整内容见上方文件卡片，可点击展开）'
                }
                const unreadable = (note) => '解析后保存路径：' + path + '（无法按文本读取：' + (note || '未知原因') + '）'
                try {
                    ...
                    return '解析后保存路径：' + path + '（解析失败：' + (messageOf(error) || '未知原因') + '）'
                }
            },
        })), 'dsh-omnifile.tool')
    }
