...
import {KIND_DOC, KIND_IMAGE, KIND_MEDIA, KIND_OTHER, KIND_TEXT, MARKER_UNKNOWN, markerText, messageOf, NAMESPACE} from './common.js'

export const name = 'dsh-omnifile'
export const inject = ['webServer', 'sessions', 'tools', 'settings', 'credentials', 'llm']

/* 消息标记 / 源文件名引用 / 组装 / messageOf 等双端共用元素统一来自 lib/common.js
 * （唯一来源：改动 MARKER_* 或 markerText 时只需改 common.js，宿主与客户端自动一致）。
 * NAMESPACE 同样来自 common.js，此处只保留宿主私有常量。 */
...
export const Config = z.object({
    ...
    timeoutMs: z.number(60000).description('单次多模态调用的超时（毫秒）'),
    maxFileBytes: z.number(50 * 1024 * 1024).description('单个上传文件大小上限'),
    maxDocImages: z.number(8).description('单个文档最多交给多模态识别的内嵌图片数'),
    docMaxChars: z.number(120000).description('文档转 Markdown 后保留的最大字符数（超出部分截断）'),
    concurrency: z.number(1).description('调用多模态模型的并发数（默认 1）'),
    temperature: z.number(0.7).description('多模态模型采样温度（0-2，默认 0.7）'),
    topP: z.number(1).description('多模态模型 nucleus 采样 top_p（0-1）'),
    maxTokens: z.number(8192).description('多模态模型单次输出最大 token 数（默认 8192）'),
    /* 上限类参数（可在设置界面配置） */
    describeCacheMax: z.number(300).description('多模态识图结果缓存条数上限（LRU，默认 300）'),
    listMaxFiles: z.number(2000).description('@ 文件选择器最多列出工作区文件数（默认 2000）'),
    listMaxDepth: z.number(12).description('@ 文件选择器递归遍历最大深度（默认 12）'),
    maxNameChars: z.number(120).description('文件名清洗后的最大长度，字符（默认 120）'),
    maxBatchImages: z.number(20).description('一次粘贴/拖拽最多放入原生附件的图片数（客户端，默认 20）'),
    progressPollMs: z.number(400).description('解析进度轮询间隔，毫秒（客户端，默认 400）'),
})
...
export function apply(ctx, config = {}) {
    ...
    if (webServer !== undefined) {
        /* 把 lib/common.js 原文按 ESM 提供给浏览器端 client.js（dynamic import 用）：
         * DSH 只把一个客户端 bundle 交给浏览器，无法 require 兄弟文件，故由宿主挂这个
         * 路由返回 common.js 源码；改动共用的常量/字符串/函数只需改 common.js 一处。 */
