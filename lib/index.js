 ...
 * 2. POST  /api/omnifile/process  用 @firecrawl/anydoc 解析文档，提取内嵌图片交给多模态模型识别，组合结果（可选 token 上报进度）
 * 3. GET   /api/omnifile/status   按 token 查询处理实时进度（客户端轮询，多模态识别期间显示阶段）
 * 4. POST  /api/omnifile/describe 单张图片交给多模态模型识别（带内容哈希缓存）
 * 5. POST  /api/omnifile/open     用本地默认程序打开文件（预览）
 * 6. GET   /api/omnifile/file     会话内文件预览/缩略图（仅允许会话 uploads 与工作区内的路径）
 * 7. GET   /api/omnifile/parsed   按源文件路径返回解析结果全文（<uploads>/<原名>.parsed.md，折叠卡片懒加载用）
 * 8. GET   /api/omnifile/models   枚举「设置-模型」已配置且支持 image 的提供商/模型（供设置页下拉点选）
 * 9. 变体提供商 omnifile-<upstream>：把文本-only 主模型的图片块改写为多模态模型生成的文字描述（wire-only）
 * 10. 注册 omnifile 工具：主模型可自行解析本地文件
 *
...
function effectivePrompt(cfg, prompt) {
    return typeof prompt === 'string' && prompt !== '' ? prompt : (cfg.describePrompt || '请按要求描述这张图片。')
}
...
/**
 * 由源文件路径推导解析结果路径（<workspace>/uploads/<原文件名去扩展名>.parsed.md）。
 * 折叠卡片懒加载与写入共用这一条命名规则，保证两端一致。
 */
function parsedMarkdownPath(cwd, sourcePath) {
    const base = sanitizeName(basename(sourcePath)).replace(/\.[^.]+$/, '') || 'file'
    return join(resolve(cwd), 'uploads', base + '.parsed.md')
}

/** 把解析出的 Markdown 落盘到 <uploads>/<basename>.parsed.md，供前端折叠卡片懒加载。 */
async function writeParsedMarkdown(cwd, sourcePath, markdown) {
    try {
        const parsedPath = parsedMarkdownPath(cwd, sourcePath)
        await fs.mkdir(join(resolve(cwd), 'uploads'), {recursive: true})
...
async function processDocument(ctx, cfg, cwd, filePath, fileName, limitImages, onProgress) {
            ...
            if (fallback.images.length > 0) {
                ...
                await Promise.all(chosen.map(async (img, index) => {
                    try {
                        ...
                        const description = await limitPdf(() => describeImageCached(ctx, cfg, assetPath, cfg.describePrompt || '请按要求描述这张图片。'))
                        savedImages.push({
                            path: assetPath,
    ...
    await Promise.all(chosen.map(async (asset, index) => {
        try {
            ...
            const description = await limit(() => describeImageCached(ctx, cfg, assetPath, cfg.describePrompt || '请按要求描述这张图片。'))
        ...
        for (const block of blocks) {
            ...
            const replacement = await limit(async () => {
                try {
                    ...
                    const description = await describeImageCached(this.ctx, cfg, pathEvidence.path, cfg.describePrompt || '请按要求描述这张图片。')
                    return {
                        type: 'text',
...
export function apply(ctx, config = {}) {
            ...
            handler: async (req, res) => {
                ...
                try {
                    ...
                    if (kind === 'doc') {
                        ...
                        // 解析结果落盘为 <uploads>/<name>.parsed.md，前端折叠卡片懒加载用
                        const parsedPath = await writeParsedMarkdown(cwd, target, result.markdown)
                    ...
                    try {
                        ...
                        return writeJson(res, 200, {
                            ...
                            path: target,
                            size: stat?.size,
                            note: '无法解析该文件（' + messageOf(error) + '）。已忽略，绝对路径提供给模型备用。',
                        })
                    }
                } catch (error) {
        ...
        ctx.effect(() => webServer.register({
            ...
            handler: async (req, res) => {
                try {
                    ...
                    const text = await describeImageCached(ctx, cfg, target, prompt)
                    return writeJson(res, 200, {ok: true, kind: 'image', text, path: target})
                } catch (error) {
                    return writeJson(res, 500, {ok: false, error: messageOf(error)})
                }
            },
        }), 'dsh-omnifile.describe')
        ...
        ctx.effect(() => webServer.register({
            ...
            handler: async (req, res) => {
                try {
                    ...
                    /* 由源文件路径推导解析结果（<uploads>/<原名>.parsed.md），返回全文供折叠卡片展开。 */
                    const parsedPath = parsedMarkdownPath(cwd, target)
    ...
    if (tools !== undefined) {
        ctx.effect(() => tools.register(defineTool({
            name: 'omnifile',
            description: "分析本地文件并返回其内容。支持的类型与行为：\n1）图片（.png/.jpg/.jpeg/.webp/.gif/.bmp/.svg/.avif）：交给配置的多模态模型识别为文字描述；同一张图片的结果按内容哈希缓存，不会重复调用多模态服务。若当前模型本身支持 image 输入，直接用 read_image 查看原图更合适（避免重复处理），omnifile 的图片模式适合文本-only 模型或需逐字转写的场景。\n2）文档（.doc/.docx/.docm/.odt/.pdf/.ppt/.pptx/.rtf/.epub/.xlsx/.ods/.odp/.csv 及 .pps/.pot/.pptm/.ppsx/.ppsm/.xls/.xlsm/.xlsb 等变体）：用 @firecrawl/anydoc 转为 GitHub-Flavored Markdown，并识别其中的内嵌图片；纯扫描/图像型 PDF 会用 PyMuPDF 逐页渲染识别。\n3）文本（.json/.txt/.md/.html/.shtml）：直接以 UTF-8/GB18030 解码为文字（JSON 美化、HTML 剥标签）。\n4）其它（音视频等二进制）：返回文件路径与大小等元信息，不做内容解析（二进制无法按文本读取）。\n说明：需先通过「设置-模型」配置一个支持 image 输入的多模态模型并在此插件设置里选择它，否则图片/文档内嵌图片识别会报错。",
            parameters: {
