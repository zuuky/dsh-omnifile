    ...
    if (tools !== undefined) {
        ctx.effect(() => tools.register(defineTool({
            ...
            description: "分析本地文件并返回其内容。支持的类型与行为：\n" +
                "1）图片（.png/.jpg/.jpeg/.webp/.gif/.bmp/.svg/.avif）：交给配置的多模态模型识别为文字描述；同一张图片的结果按内容哈希缓存，不会重复调用多模态服务。若当前模型本身支持 image 输入，直接用 read_image 查看原图更合适（避免重复处理），dshomnifile 的图片模式适合文本-only 模型或需逐字转写的场景。\n" +
