...
export const Config = z.object({
    ...
    temperature: z.number().default(0).description('多模态模型采样温度（0-2，0 最确定）'),
    topP: z.number().default(1).description('多模态模型 nucleus 采样 top_p（0-1）'),
    maxTokens: z.number().default(2048).description('多模态模型单次输出最大 token 数'),
})
...
async function describeImageCached(ctx, cfg, imagePath, prompt) {
    ...
    const baseUrl = String(cfg.provider.baseUrl || DEFAULT_BASE_URL).replace(/\/+$/, '')
    const model = cfg.provider.model || DEFAULT_MODEL
...
export function apply(ctx, config = {}) {
    ...
    if (tools !== undefined) {
        ctx.effect(() => tools.register(defineTool({
            name: 'omnifile',
            description: '分析本地文件：图片交给配置的多模态模型识别为文字描述（同一张图片的结果会被内容哈希缓存，不会重复调用多模态服务）；文档（Word/PowerPoint/Excel/PDF/EPUB/RTF/CSV/OpenDocument）用 @firecrawl/anydoc 转换为 GitHub-Flavored Markdown 并识别内嵌图片；文本（JSON/TXT/MD/HTML/SHTML）直接解码为文字；其他文件返回路径信息。注意：若当前模型支持 image 输入，图片也可以用 read_image 工具直接查看原图，此时无需再调用 omnifile 识别图片，避免重复处理；omnifile 的图片模式适合文本-only 模型或需要逐字转写的场景。',
            parameters: {
