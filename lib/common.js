 ...
 *
 * 两端如何拿到这份共用元素：
 *  - 宿主端 lib/index.js（Node ESM）：静态 import { ... } from './common.js'；
 *  - 浏览器端 lib/client.js：DSH 只把一个客户端 bundle（lib/client.js）交给浏览器，
 *    其中 require() 只能解析已注册模块、无法加载本文件的兄弟模块，所以由宿主在
 *    /api/omnifile/common.js 挂一个路由把本文件原文按 ESM 返回，client.js 再 dynamic
 *    import('/api/omnifile/common.js') 拿到同一份导出。
 *
...
/**
 * 组装「解析后保存路径」标记（两端共用）。
 * @param {string} path - 保存路径（成功为 <uploads>/<源文件名>.md，失败/不可读为源路径）。
 * @param {object} [opts]
 * @param {boolean|string} [opts.ok] - true=成功；false=解析失败；字符串或缺省=该状态词
 *   （传 '无法按文本读取' 或略过 → 不可读；传任意其它状态词直接沿用）。
 * @param {string} [opts.note] - 状态后的原因说明（失败/不可读时）。
 * @param {string} [opts.source] - 成功时附带的「源文件」绝对路径回指（与 path 不同才附加）。
 * @returns {string} 一行可读标记。
 */
export function markerText(path, {ok = MARKER_STATUS_UNREADABLE, note, source} = {}) {
    const p = String(path || '')
    const isOk = ok === true || ok === MARKER_STATUS_OK
    if (isOk) {
        const sourceTail = (typeof source === 'string' && source !== '' && source !== p)
            ? '；' + MARKER_SOURCE_TAG + source
            : ''
        return MARKER_PREFIX + p + '（' + MARKER_STATUS_OK + sourceTail + '）'
    }
    const status = ok === false ? MARKER_STATUS_FAILED : (typeof ok === 'string' && ok !== '' ? ok : MARKER_STATUS_UNREADABLE)
    const noteText = typeof note === 'string' && note !== '' ? '：' + note : ''
 ...
 * @param {string} [statusTail] - 括号内状态文本。
 * @returns {string|undefined}
 */
export function sourcePathOf(statusTail) {
